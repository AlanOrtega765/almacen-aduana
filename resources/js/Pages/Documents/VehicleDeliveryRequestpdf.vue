...
const props = defineProps(["ServicioentVehiculo"]);
    ...
    <div class="pt-20">
        <header class="flex items-center justify-center gap-10 text-center">
            <div class="relative flex">
                ...
                <h1 class="text-2xl font-bold">SERVICIO DE ENTRA DE VEHICULOS</h1>
            </div>
        </header>
        ...
        <main>
            <div>
                <h1 class="text-right text-4xl font-bold mt-10">
                    N° {{ ServicioentVehiculo.n_boleta }}
                </h1>
                <div class="flex flex-col w-full items-end uppercase">
                    <span>FECHA C.R.: {{ ServicioentVehiculo.fecha_reten }}</span>
                    <span>AVANZADA: {{ ServicioentVehiculo.avanzada }}</span>
                    <span>ALMACÉN: BODEGA {{ ServicioentVehiculo.nombre_almc }}</span>
                </div>
                <div class="flex flex-col uppercase">
                    <span
                        >PASAJERO:
                        {{
                            ServicioentVehiculo.nombre_p + " " + ServicioentVehiculo.apellido_p
                        }}</span
                    >
                    <span
                        >RUT / DNI / PASAPORTE:
                        {{ ServicioentVehiculo.nro_id_person }}</span
                    >
                    <span>NACIONALIDAD: {{ ServicioentVehiculo.nacionalidad_p }}</span>
                    <span>DOMICILIO: {{ ServicioentVehiculo.direccion_p }}</span>
                    <span>CIUDAD: {{ ServicioentVehiculo.ciudad_p }}</span>
                </div>
            ...
            <div :class="merchandises.length >= 10 ? 'relative' : 'absolute'" class="bottom-0 w-full mt-20">
                ...
                <div class="flex flex-col">
                    <div class="flex justify-between mb-5">
                        <h5 class="font-bold">
                            Uso de Franquicia:
                            <span class="font-normal" v-html="ServicioentVehiculo.franquicia === 1 ? 'SI' : 'NO'"></span>
                        </h5>
                        <h5 class="font-bold">
                            Peso: <span class="font-normal">{{ ServicioentVehiculo.peso }} Kg.</span>
                        </h5>
                        <h5 class="font-bold">
                            Bultos: <span class="font-normal">{{ ServicioentVehiculo.cantidad_bulto }}</span>
                        </h5>
                    </div>
                    <div class="w-full">
                        ...
                        >{{ ServicioentVehiculo.observaciones }}</div>
                    </div>
                    <div class="flex flex-col">
                        ...
                        >
                        <span class="font-bold"
                            >Plazo Máximo de Retiro: {{ ServicioentVehiculo.fecha_venc }}</span
                        >
                    </div>
                </div>
            </div>
